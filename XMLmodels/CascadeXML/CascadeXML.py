...
def get_bert(bert_name):
    if 'deberta_large' in bert_name:
        print('load deberta-large')
        model_config = DebertaConfig.from_pretrained('microsoft/deberta-large')
        model_config.output_hidden_states = True
        bert = DebertaModel.from_pretrained('microsoft/deberta-large', config=model_config)
    ...
    elif 'bert_large' in bert_name:
        print('load bert_large')
        model_config = BertConfig.from_pretrained('google-bert/bert-large-uncased')
        model_config.output_hidden_states = True
        bert = BertModel.from_pretrained('google-bert/bert-large-uncased', config=model_config)
    elif 'deberta' in bert_name:
        print('load deberta')
        model_config = DebertaConfig.from_pretrained('microsoft/deberta-base')
        model_config.output_hidden_states = True
        bert = DebertaModel.from_pretrained('microsoft/deberta-base', config=model_config)
...
class CascadeXML(nn.Module):
    def __init__(self, params, train_ds, device):
        ...
        if "large" in self.base_model:
            if len(self.num_ele) == 4:
                self.layers = [(15, 16), 18, 22, 24]
                # self.layers = [(8, 9), 10, 11, 12] #other option
            elif len(self.num_ele) == 3:
                self.layers = [(16,18), 22, 24]
            elif len(self.num_ele) == 5:
                self.layers = [(12, 14),18, 20,22,24]
            elif len(self.num_ele) == 2:
                self.layers = [ (18,20),24]
        else:
            if len(self.num_ele) == 4:
