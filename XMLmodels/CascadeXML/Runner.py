...
################################
    ...
    def predict_f1(self, preds, probs, y_true):
        #print(f"preds:{len(preds)} {preds} \n\n  probs: {len(probs)} {probs} \n\n y_true: {len(y_true)}{y_true}")
        
        #Filter predictions with probabilities higher than 0.5
        mask = probs > 0.5
        #filtered_preds = preds * mask
        filtered_preds = preds * mask + (-1) * (~mask)
        #print(filtered_preds)
        filtered_preds_list = [row[row > -1] for row in filtered_preds]
        #print(filtered_preds_list)

        total_tp, total_fp, total_fn = 0,0,0

        # Iterate through each pair of prediction and ground truth
        for pred, truth in zip(filtered_preds_list, y_true):
            # Convert both to sets to make the comparison easier
            pred_set = set(pred.numpy())
            truth_set = set(truth.numpy())

            self.all_preds_list.append(list(pred_set))
            #self.all_probs_list.append(list(truth_set))
            self.all_true_list.append(list(truth_set))

            #print("pred", pred_set, "\n")
            #print("truth", truth_set, "\n")
            """
            # True Positives (common elements between prediction and truth)
            TP = len(pred_set & truth_set)
            #print(f"true pos: {TP}")
            # False Positives (elements in prediction but not in truth)
            FP = len(pred_set - truth_set)
            #print(f"false pos: {FP}")
            # False Negatives (elements in truth but not in prediction)
            FN = len(truth_set - pred_set)
            #print(f"false neg: {FN}")
            # Update total counts
            self.counts_f1['true_pos'] += TP
            self.counts_f1['false_pos'] += FP
            self.counts_f1['false_neg'] += FN
            """
    ...
    def predict_at_k(self, preds, y_true, i,  ext_adj = None):
        ...
        for pred, tr in zip(preds, y_true):
            ...
            #print(len(pred))
            #print(len(torch.topk(pred, 2)))
            #print(len(tr))
            #print(match.numpy())
            #match = math.floor(match / len(tr))
...
#################################     
    ...
    def fit_one_epoch(self, model, params, device, epoch):
        ...
        if isinstance(model,  nn.parallel.DistributedDataParallel):
            ...
            self.counts_adj = [0 for _ in range(len(model.module.clusters)+1)]
        else:
        ...
        #print(f"Länge dataloader: {len_dl}")

        self.optimizer.zero_grad()

        if params.local_rank==0:
            print(f'\nStarting Epoch: {epoch}\n')
            pbar = tqdm(self.train_dl, desc=f"Epoch {epoch}", disable = True)
        else:
            pbar = self.train_dl
        ...
        print(f"Train time : {epoch_time} seconds")
        self.total_train_time += epoch_time
        print(f"Total train time: {self.total_train_time} seconds, {self.total_train_time/60} minutes")
    ...
    def test(self, model, params, device, epoch=0):
        ...
        if isinstance(model,  nn.parallel.DistributedDataParallel):
            self.counts = [torch.zeros(self.top_k, dtype=int).to(device) for _ in range(len(model.module.clusters)+1)]
            #self.counts_f1 = {'true_pos': 0, 'false_pos': 0, 'false_neg': 0}
            self.all_preds_list = []
            self.all_probs_list = []
            self.all_true_list = []
            self.counts_adj = [0 for _ in range(len(model.module.clusters)+1)]
        ...
        for step, sample in enumerate(pbar): 
            ...
            # all_preds = [torch.topk(probs, self.top_k)[1].cpu() for probs in all_probs] # Old 
            all_preds_probs = [torch.topk(probs, self.top_k) for probs in all_probs] #new and next 2 lines new
            all_preds = [preds[1].cpu() for preds in all_preds_probs]  # Top-k Labels
            all_probabs = [preds[0].cpu() for preds in all_preds_probs]  # Top-k Wahrscheinlichkeiten
            ...
            #print(f"idx: {idx}\n\n  cands: {cands} \n\n len_cprobs:{len(cprobs)} cprobs: {cprobs}")

            #print("before", self.counts, self.counts_f1)
            ...
            for i, (preds, w_preds,  label) in enumerate(zip(all_preds, all_weighted_preds,  labels)):
                ...
                if i > 0:
                    ...
                    self.predict_at_k(preds, label, i)

            #print(self.counts_f1['true_pos'],self.counts_f1['false_pos'],self.counts_f1['false_neg'])
            #New Start 
            zipped_data = list(zip(all_preds, all_probabs, labels))
            (preds, probs, label) = zipped_data[-1]
            self.predict_f1(preds, probs, label)

            #New End 
            #print(self.counts_f1['true_pos'],self.counts_f1['false_pos'],self.counts_f1['false_neg'])


            #print("after", self.counts)

            #for i, (preds, w_preds, label) in enumerate(zip(all_preds, all_weighted_preds, labels)):s
            #    self.predict(preds, label, i)
            #    if i > 0:
            #        self.predict(w_preds, label, i, self.weighted_counts)
        ...
        if params.local_rank == 0: 
            ...
            y_true_bin = sp.lil_matrix((len(self.all_true_list), params.num_labels), dtype=np.int8)
            y_preds_bin = sp.lil_matrix((len(self.all_preds_list), params.num_labels), dtype=np.int8)
            
            # Populate matrices
            for i, labels in enumerate(self.all_true_list):
                y_true_bin[i, labels] = 1
            for i, labels in enumerate(self.all_preds_list):
                y_preds_bin[i, labels] = 1
            
            # Convert to CSR for efficient operations
            y_true_bin = y_true_bin.tocsr()
            y_preds_bin = y_preds_bin.tocsr()
            
            # Compute F1 scores
            sk_f1_macro = f1_score(y_true_bin, y_preds_bin, average='macro')
            sk_f1_micro = f1_score(y_true_bin, y_preds_bin, average='micro')
            sk_f1_sample = f1_score(y_true_bin, y_preds_bin, average='samples')
            sk_acc_score = accuracy_score(y_true_bin, y_preds_bin)
            #f1_samples = f1_score(y_true_bin, y_preds_bin, average='samples')

            """
            #old f1 score, high memory usage
            # Binarize the labels
            mlb = MultiLabelBinarizer()
            mlb.fit(self.all_true_list + self.all_preds_list)  # Fit on all possible labels
            
            y_true_binold = mlb.transform(self.all_true_list)
            y_preds_binold = mlb.transform(self.all_preds_list)

            sk_f1_microold = f1_score(y_true = y_true_binold, y_pred = y_preds_binold, average = 'micro')
            sk_f1_macroold = f1_score(y_true = y_true_binold, y_pred = y_preds_binold, average = 'macro')
            """
            

            if sk_f1_micro > self.best_f1_micro: 
                self.best_f1_micro = sk_f1_micro
                self.patience = 0
            else: 
                self.patience += 1
            ...
            try: 
                prec_micro = self.counts_f1['true_pos']/(self.counts_f1['true_pos']+self.counts_f1['false_pos'])
                recall_micro = self.counts_f1['true_pos']/(self.counts_f1['true_pos']+self.counts_f1['false_neg'])
                f1_micro = (2*prec_micro*recall_micro)/(recall_micro+prec_micro)
            except ZeroDivisionError:
                prec_micro = recall_micro = f1_micro = 0 
             """
            ...
            ######################################
            p_adj = [(sumScore/self.num_test) * 100.0 for sumScore in self.counts_adj]
            ####################################
            ...
            if self.top_k == 20: 
                for i in range(len(precs)):
                    #print(f'Level-{i} Test Recall: R@1: {recall[0]:.2f}')
                    print(f'Level-{i} Test Scores: P@1: {precs[i][0]:.2f}, P@2: {precs[i][1]:.2f}, P@3: {precs[i][2]:.2f}, P@5: {precs[i][4]:.2f}, P@10: {precs[i][9]:.2f}, P@20: {precs[i][19]:.2f}')
                    if i != 0:
                        print(f'Level-{i} Weighted Test Scores: P@1: {weighted_precs[i][0]:.2f}, P@2: {weighted_precs[i][1]:.2f}, P@3: {weighted_precs[i][2]:.2f}, P@5: {weighted_precs[i][4]:.2f}')
                    print("----------------------------------------")
            else:
                for i in range(len(precs)):
                    #print(f'Level-{i} Test Recall: R@1: {recall[0]:.2f}')
                    print(f'Level-{i} Test Scores: P@1: {precs[i][0]:.2f}, P@2: {precs[i][1]:.2f}, P@3: {precs[i][2]:.2f}, P@5: {precs[i][4]:.2f}')
                    if i != 0:
                        print(f'Level-{i} Weighted Test Scores: P@1: {weighted_precs[i][0]:.2f}, P@2: {weighted_precs[i][1]:.2f}, P@3: {weighted_precs[i][2]:.2f}, P@5: {weighted_precs[i][4]:.2f}')
                    print("----------------------------------------")
                
                
            print(f"Level-{i} Weighted PSP Score: PSP@1: {psp[0]:.2f}, PSP@2: {psp[1]:.2f}, PSP@3: {psp[2]:.2f}, PSP@5: {psp[4]:.2f}")
            print(f"R-Precision: {p_adj[i]}")
            print(f"F1-micro: {sk_f1_micro}")
            print(f"F1-macro: {sk_f1_macro}")
            print(f"F1-sample: {sk_f1_sample}")
            print(f"Mean-Accuracy: {sk_acc_score}")
    ...
    def test_ensemble(self, params):
        ...
        self.counts_adj = [torch.zeros(self.top_k, dtype=int)]
