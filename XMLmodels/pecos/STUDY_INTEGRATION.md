...
The integration retains parameter sets and launch scripts for HTC and XMC
datasets; BERT, RoBERTa and XLNet variants and their ensembles; cross-domain
conversion under `XMLPreprocessing/XR-Transformer`; top-20 output; R-Precision
and F1 reporting; and compact per-seed and aggregate historical result files.
Release fixes give the converter a validated CLI with a shared train/test label
dimension, calculate ensemble F1 from sparse supports, use the requested
ensemble method consistently, restore the example evaluator to valid Python,
and harden launch/evaluation scripts. R-Precision now rejects empty-gold rows
and handles shorter prediction rows safely.
...
The retained compact outputs are historical. Some average files repeat seed
records or have zero deviations, and they do not reliably reproduce the final
table entries. The authoritative paper values are in
[`RESULTS.md`](../../RESULTS.md), not in those integration outputs.
