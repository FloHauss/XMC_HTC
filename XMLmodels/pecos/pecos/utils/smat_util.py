...
class CsrEnsembler(object):
    ...
    @staticmethod
    def print_ens(Ytrue, pred_set, param_set, ens_method="rank_average", topk=20):
...
# changed: add r_prec
class Metrics(collections.namedtuple("Metrics", ["prec", "recall", "r_prec"])):
    """The metrics (precision, recall, r-precision) for multi-label classification problems."""
    ...
    def default(cls):
        ...
        # changed: add r_prec
        return cls(prec=[], recall=[], r_prec=0)
    ...
    def generate(cls, tY, pY, topk=10):
        ...
        # changed: add r_prec
        return cls(prec=prec, recall=recall, r_prec=r_prec)