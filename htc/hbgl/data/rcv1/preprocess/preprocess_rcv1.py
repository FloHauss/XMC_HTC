...
def might_extract_tar(path):
    ...
    if path.suffixes[-2:] == ['.tar', '.gz']:
        dir_name = '.'.join(path.name.split('.')[:-2])  # Removes '.tar.gz' to get the directory name
    else:
        dir_name = path.stem  # Fallback, should not be needed if all files are '.tar.gz'
    dir_output = path.parent / dir_name
    if not dir_output.exists():
        if path.exists():
            with tarfile.open(str(path), "r:gz") as tf:  # "r:gz" indicates gzip compressed tar file
...
if __name__ == '__main__':
    ...
    parser.add_argument('root_dir', type=str, help='Absolute path to directory which contains rcv1.tar.gz')
    ...
    logging.info('Extracting main dataset from rcv1.tar.gz')
    might_extract_tar(ROOT / 'rcv1.tar.gz')
