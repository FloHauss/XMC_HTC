...
def main(flags=None):
    ...
    for model_recover_path in glob.glob(args.model_path):
        ...
        with tqdm(total=total_batch) as pbar:
            ...
            logger.info('{}'.format(batch_count))
            first_batch = True
            while next_i < len(input_lines):
                ###CUSTOM
                logger.info('{},{}'.format(next_i, len(input_lines)))
        ...
        if (args.ignore_meta_label):
            valid_labels = set()
            for k in label_map:
                if 'meta_label' not in k.lower():
                    valid_labels.add(token_to_id(label_map[k]))

            predict_labels = [[label for label in l if label in valid_labels] for l in predict_labels]
