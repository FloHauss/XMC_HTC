...
def _precision_at_k(epoch_predicts, epoch_labels, k):
    """Precision@K averaged over samples."""
    if len(epoch_predicts) != len(epoch_labels):
        raise ValueError("Predictions and labels must contain the same number of samples.")
    if not epoch_predicts:
        raise ValueError("At least one sample is required.")
    if k <= 0:
        raise ValueError("k must be positive.")

    total = 0.0
    for preds, golds in zip(epoch_predicts, epoch_labels):
        if len(preds) < k:
            raise ValueError(f"Cannot compute P@{k} with only {len(preds)} labels.")
        np_preds = np.array(preds, dtype=np.float32)
        top_k_idx = set(np.argsort(-np_preds)[:k])
        gold_set = set(golds)
        total += len(top_k_idx & gold_set) / k
    return total / len(epoch_predicts)


def _r_precision(epoch_predicts, epoch_labels):
    """R-Precision averaged over samples."""
    if len(epoch_predicts) != len(epoch_labels):
        raise ValueError("Predictions and labels must contain the same number of samples.")
    if not epoch_predicts:
        raise ValueError("At least one sample is required.")
    ...
    for preds, golds in zip(epoch_predicts, epoch_labels):
        r = len(golds)
        if r == 0:
            raise ValueError("R-Precision is undefined for a sample without gold labels.")
        np_preds = np.array(preds, dtype=np.float32)
        top_r_idx = set(np.argsort(-np_preds)[:r])
        gold_set = set(golds)
        total += len(top_r_idx & gold_set) / r
    return total / len(epoch_predicts)
...
def evaluate(epoch_predicts, epoch_labels, id2label, threshold=0.5, top_k=None):
    ...
    p1 = _precision_at_k(epoch_predicts, epoch_gold, 1)
    p3 = _precision_at_k(epoch_predicts, epoch_gold, 3)
    p5 = _precision_at_k(epoch_predicts, epoch_gold, 5)
    r_prec = _r_precision(epoch_predicts, epoch_gold)
