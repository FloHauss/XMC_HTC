...
# fairseq 0.10 still imports NumPy aliases removed in NumPy 1.24. Keep this
# compatibility shim before importing fairseq. The mapping is explicit so it
# cannot evaluate arbitrary strings.
...
if __name__ == '__main__':
    ...
    model.to(device)
    tracker = CostTracker()
    tracker.record_model(model)
    tracker.start_training()
    model_params = tracker.model_parameters
    print(f'[cost] params_total={model_params["model_params_total"]:,}'
          f'  params_trainable={model_params["model_params_trainable"]:,}')
    ...
    for epoch in range(1000):
        ...
        pbar.close()
        tracker.end_train_phase(len(train.dataset))
        ...
        tracker.end_val_phase(epoch, scores)
    ...
    tracker.print_training_summary()
    metrics_path = os.path.join('checkpoints', args.name, 'cost_metrics.json')
    tracker.save(metrics_path, extra={'run_name': args.name, 'dataset': args.data})
